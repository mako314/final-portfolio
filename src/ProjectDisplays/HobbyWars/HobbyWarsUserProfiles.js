...
<div className="w-full lg:w-full mt-4 mb-4 py-4 px-4">
    ...
    <p className={`text-${theme === 'dark' ? 'white' : 'gray-800'}  font-bold`}><HobbyWarLogo/> Max Adler - </p>
    <p className={`text-sm text-${theme === 'dark' ? 'white' : 'gray-700'}  font-bold`}>TOTALLY Real Human</p>
    </div>

        {/* Right Column for Character Description */}
        <div className="flex-auto" style={{ flexBasis: '85%' }}> {/* Takes up remaining 40% width */}
            <div className={`p-4 bg-${theme === 'dark' ? 'gray-100':'gray-200'} rounded-md shadow`}>
                <p className={`text-center ${theme === 'dark' ? 'text-gray-700':'text-gray-600'}  `}>
