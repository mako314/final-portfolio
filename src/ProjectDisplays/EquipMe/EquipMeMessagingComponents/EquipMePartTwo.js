...
function EquipMeMessagingOwnerToUser({openModal, goToPreviousPage, goToNextPage, images, selectedIndex, selectedImage, closeModal, handleModalContentClick, handleOverlayClick, isModalOpen, setSelectedIndex}) {
  ...
  return (
    ...
    <h2 className={`text-${theme === 'dark' ? 'white' : 'black'} mb-8 text-3xl font-bold md:mb-12 md:text-4xl lg:mb-16 lg:text-5xl`}>Batch Upload Your Equipment<br />Supporting CSV, XML, XLSX Files</h2>
      ...
      {selectedImage?.src && (
        ...
        <span className="flex max-w-sm rounded-md p-4 text-[#222222] [box-shadow:rgba(0,_0,_0,_0.05)_0px_0px_10px] cursor-pointer" onClick={() => changeIndex(1)}>
          ...
          <span className={`flex max-w-sm rounded-md p-4 text-white bg-${theme === 'dark' ? 'gray-700' : 'white'} border-l-4 border-${theme === 'dark' ? 'blue-500' : 'black'} shadow-${theme === 'dark' ? '2xl' : 'md'} cursor-pointer`} onClick={() => changeIndex(2)}>
            <img 
