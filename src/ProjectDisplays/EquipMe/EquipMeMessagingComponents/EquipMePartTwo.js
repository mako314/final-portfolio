...
function EquipMeMessagingOwnerToUser({openModal, goToPreviousPage, goToNextPage, selectedImage, closeModal, handleModalContentClick, handleOverlayClick, setSelectedIndex}) {
  ...
  return (
    ...
    <h2 className={`text-${theme === 'dark' ? 'white' : 'black'} dark:text-white mb-8 text-3xl font-bold md:mb-12 md:text-4xl lg:mb-16 lg:text-5xl`}>Batch Upload Your Equipment<br />Supporting CSV, XML, XLSX Files</h2>
      
      <div className="flex flex-col gap-8 lg:flex-row lg:gap-20">
      <div className="relative group cursor-pointer">
      
      <img alt="Equip Me Owner CSV Upload" src={EquipMeOwnerCsvUpload} className="inline-block h-full max-h-[428px] w-full max-w-md object-fill lg:max-w-2xl" />
      
      <div onClick={() => openModal(EquipMeOwnerCsvUpload, "EquipMe CSV/XML/ XLSX Uploader", "Equip Me Owner CSV Upload")} className="rounded-xl absolute inset-0 bg-black bg-opacity-50 flex justify-center items-center group-hover:opacity-100 opacity-0 transition-opacity"> 
      ...
      {selectedImage?.src && (
        ...
        <span className="flex max-w-sm rounded-md p-4 text-[#222222] [box-shadow:rgba(0,_0,_0,_0.05)_0px_0px_10px] cursor-pointer" onClick={() => changeIndex(1)}>
          ...
          <span className={`flex max-w-sm rounded-md p-4 text-white bg-${theme === 'dark' ? 'gray-700' : 'white'} dark:bg-gray-700 border-l-4 border-${theme === 'dark' ? 'blue-500' : 'black'} dark:border-blue-500 shadow-${theme === 'dark' ? '2xl' : 'md'} dark:shadow-2xl cursor-pointer`} onClick={() => changeIndex(2)}>
            <img 
