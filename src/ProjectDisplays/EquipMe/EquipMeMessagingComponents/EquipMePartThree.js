...
function EquipMeMessagingUserToOwner({openModal, goToPreviousPage, goToNextPage, selectedImage, closeModal, handleModalContentClick, handleOverlayClick, setSelectedIndex}) {
  ...
  return (
    ...
    <h2 className={`text-${theme === 'dark' ? 'white' : 'black'} dark:text-white mb-8 text-3xl font-bold md:mb-12 md:text-4xl lg:mb-16 lg:text-5xl`}>Effortless Rental Management, All in One Place</h2>
      ...
      {selectedImage?.src && (
        ...
        <div className="flex flex-col gap-4">
          ...
          <span className={`flex max-w-sm rounded-md p-4 text-white bg-${theme === 'dark' ? 'gray-700' : 'white'} dark:bg-gray-700 border-l-4 border-${theme === 'dark' ? 'blue-500' : 'black'} dark:border-blue-500 shadow-${theme === 'dark' ? '2xl' : 'md'} dark:shadow-2xl cursor-pointer`} onClick={() => changeIndex(3)}>
            <img 
